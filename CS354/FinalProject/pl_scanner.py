...
class Scanner(object):
    ...
    operators = {'=', '+', '-', '*', '/',
                 '(', ')', ';', '<', '<=', '>', '>=', '<>', '==', '&'}
    keywords = {'rd', 'wr', 'if', 'then', 'else',
                'while', 'do', 'begin', 'end', 'def', 'pointer'}
    token = ""
    lexeme = ""
    ...
    def nextKwId(self):
        ...
        self.token = Token(
            (lexeme if lexeme in self.keywords else "id"), lexeme)
    ...
    def next(self):
        ...
        if c in self.digits:
            ...
            print("illegal character " +
                  self.program[self.pos] + " at position " + str(self.pos))
            self.pos += 1
            return self.next()
        return True
